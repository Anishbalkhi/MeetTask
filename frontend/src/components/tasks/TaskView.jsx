    ...
    return (
        <div className="w-full pt-6 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4 px-2">
                ...
                <div className="bg-white/5 p-1 rounded-lg border border-white/5 flex backdrop-blur-md">
                    <button
                        onClick={() => setViewMode("list")}
                        className={`px-3 py-1.5 rounded-md text-xs font-medium flex items-center gap-2 transition-all ${viewMode === 'list' ? 'bg-white/10 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                    >
                        <FiList /> Table
                    </button>
                    <button
                        onClick={() => setViewMode("block")}
                        className={`px-3 py-1.5 rounded-md text-xs font-medium flex items-center gap-2 transition-all ${viewMode === 'block' ? 'bg-white/10 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                    >
                        <FiGrid /> Board
                    </button>
                </div>
            ...
            {filteredTasks.length === 0 ? (
                <div className="text-center py-32 text-gray-500 bg-white/5 rounded-2xl border border-white/5 border-dashed mx-2">
                    <p>No tasks found.</p>
                </div>
            ) : (
                <>
                    {viewMode === 'list' ? (
                        <div className="overflow-x-auto rounded-xl border border-white/5 bg-[#1a1b1e]/40 backdrop-blur-sm">
                        ...
                        <div className="flex gap-6 overflow-x-auto pb-4 px-2">
                            {['TODO', 'IN_PROGRESS', 'DONE'].map(status => {
                                ...
                                return (
                                    <div key={status} className="min-w-[320px] bg-[#1a1b1e]/40 backdrop-blur-sm rounded-xl border border-white/5 p-4 flex flex-col gap-4">
                                        ...
                                        <div className="flex flex-col gap-3 min-h-[100px]">
                                            {columnTasks.map(task => (
                                                <div key={task.id} className="bg-[#25262b] p-4 rounded-lg border border-white/5 hover:border-blue-500/30 transition-all cursor-pointer shadow-sm hover:shadow-lg group relative overflow-hidden">
                                                    <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gradient-to-b from-transparent via-blue-500/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                                    <p className="text-gray-200 font-medium text-sm mb-3 group-hover:text-blue-200 transition-colors">{task.title}</p>

                                                    <div className="flex items-center justify-between mt-auto">
                                                        {task.assignee ? (
                                                            <img src={`https://i.pravatar.cc/150?u=${task.id}`} className="w-5 h-5 rounded-full ring-2 ring-[#25262b]" alt="Assignee" />
                                                        ) : (
                                                            <div className="w-5 h-5 rounded-full bg-white/5 ring-2 ring-[#25262b]"></div>
                                                        )}
                                                        <div className="text-[10px] text-gray-500 bg-black/20 px-1.5 py-0.5 rounded">
                                            ...
                                            {columnTasks.length === 0 && (
                                                <div className="h-full border-2 border-dashed border-white/5 rounded-lg flex items-center justify-center text-gray-700 text-xs">
                                                    Empty
                                                </div>
                                            )}
                                        </div>
